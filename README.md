# Elden Ring Seamless Coop autoupdater

A script to make updating ERSC easier

## Seting up

go to config.json

```json
{
    "download_folder": {"Enter your default browser downloads location"},
    "target_folder": {"Enter your supposed ersc location ...\\Steam\\steamapps\\common\\ELDEN RING\\Game"},
    "ini_file_path": {"Enter your supposed ersc ersc_settings.ini location ...\\Steam\\steamapps\\common\\ELDEN RING\\Game\\SeamlessCoop\\ersc_settings.ini"}
}
```
make sure the ERSC_automanualupdater.exe also exist

make ERSC_automanualupdater.exe run on startup
